...
public class OSTIDVisualCodeNode extends SingleOutcomeNode {
    ...
    public Action process(TreeContext context) throws NodeProcessException {
        ...
        JsonValue crontoMsgJsonValue = sharedState.get(Constants.OSTID_CRONTO_MSG);
        ...
        //2. go to next
            ...
            sharedState.remove(config.visualCodeHiddenValueId());       //string
            sharedState.remove(Constants.OSTID_CRONTO_HAS_RENDERED);    //boolean
    ...
    //should be generic, no special parameters
    private void pushCrontoJS(List<Callback> returnCallback ){
        String displayScript =
                        "function CDDC_display(isStart) {" +
                        " var CDDC_timer;" +
                        " function start(countDownDate,countdownText,expiryText,imageSrc,imageAlt,imageHeight,imageLocDomID) { " +
    ...
    private void startCrontoDisplay(JsonValue sharedState, String crontURL, List<Callback> returnCallback ){
...
//                "}" +
                ...
                "   CDDC_start(%1$s,'%2$s','%3$s','%4$s','%5$s',%6$d,'%7$s');" +
                "}" +
