    ...
    public Action process(TreeContext context) {
        ...
        if (!usernameJsonValue.isString() || usernameJsonValue.asString().isEmpty()) {
            ...
            String checkActivationJSON = String.format(Constants.OSTID_JSON_CHECK_ACTIVATION,
                    ...
                    //config.checkActivationTimeout()     //param2
                    Constants.OSTID_DEFAULT_CHECK_ACTIVATION_TIMEOUT
            );
            try {
...
//                        ExceptionOutput exceptionOutput = JSON.toJavaObject(checkActivationResponseJSON, ExceptionOutput.class);
                    activationStatusEnum = ActivationStatusOutcome.error;
    ...
    //todo, ForgeRock session is shorter than TID session timeout
    //so should I also consider the user input timeout in this node?
